...
class ProfileController < ApplicationController
  ...
  def index
    ...
    @learn_courses = UserTutorial.where('user_id = ?', current_user.id).where(tutorial_id: UserTutorial.where('user_id = ?', current_user.id)).joins('INNER JOIN tutorials ON tutorials.id = user_tutorials.tutorial_id')
